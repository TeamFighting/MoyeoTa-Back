...
public class OAuthController {
	...
	public ResponseDto<TokenInfoDto> loginKakao(@RequestBody KakaoLoginParams params, HttpServletResponse response) {
		...
		Cookie cookie = createCookie(tokenInfoDto.getRefreshToken());
	...
	private Cookie createCookie(String refreshToken) {
		...
		cookie.setMaxAge(60 * 60 * 24 * 21); // 쿠키 유효 기간 (21일)
		return cookie;
	}
	 ...
	 * 네이버와 구글 로그인은 추후에 추가
	 ...
	 @PostMapping("/google") public ResponseDto<TokenInfoDto> loginGoogle(@RequestBody GoogleLoginParams params) {
	 ...
	 @PostMapping("/naver") public ResponseDto<TokenInfoDto> loginNaver(@RequestBody NaverLoginParams params) {
